...
import type { PricingResult, ListingAssessment, PriceRating } from '../pricing/types';

function fmtPrice(amount: number, currency: string): string {
  return `${currency}${amount.toFixed(2)}`;
}

const BADGE_STYLE: Record<Exclude<PriceRating, 'no-data'>, { bg: string; color: string; label: string }> = {
  good: { bg: 'rgba(92, 184, 92, 0.1)', color: '#5cb85c', label: '🟢 Good deal' },
  fair: { bg: 'rgba(240, 173, 78, 0.1)', color: '#f0ad4e', label: '🟡 Fair' },
  high: { bg: 'rgba(217, 83, 79, 0.1)', color: '#d9534f', label: '🔴 Above market' },
};

function injectBadge(card: Element, assessment: ListingAssessment, currency: string): void {
  if (!assessment.showBadge || assessment.rating === 'no-data') return;

  const style = BADGE_STYLE[assessment.rating as Exclude<PriceRating, 'no-data'>];
  const { listing, matchedGroup } = assessment;

  let container = card.querySelector('.bb-badge-container') as HTMLElement | null;
  ...
  const postageNote = listing.postage > 0
