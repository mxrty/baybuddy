...
describe('discoverIdentityVocab — model-shaped tokens', () => {
  test('always includes alphanumeric model tokens regardless of frequency', () => {
    const vocab = discoverIdentityVocab(['HP072UK air purifier', 'Some other listing']);
  ...
  test('preserves capacity tokens as identity', () => {
    const vocab = discoverIdentityVocab(['512GB storage device', 'another listing']);
...
describe('tokenize — model number preservation', () => {
  test('HP072UK is preserved as an identity token', () => {
    ...
    const result = tokenize('Shark NEVERCHANGE5 Compact Pro Air Purifier HP072UK', vocab);
  ...
  test('BP01 is preserved as an identity token', () => {
    const vocab = new Set<string>();
    const result = tokenize('Dyson BP01 Pure Cool Me Personal Air Purifier', vocab);
  ...
  test('5s model suffix is an identity token', () => {
    const vocab = new Set<string>();
    const result = tokenize('Apple iPhone 5s Silver 16GB Unlocked Smartphone', vocab);
...
describe('weightedSimilarity — identity overlap dominates', () => {
  ...
  test('pure identity overlap scores higher than pure descriptor overlap', () => {
    ...
    const descriptorScore = weightedSimilarity(sharesDescriptors, descriptorRef);
  ...
  test('empty token sets return 0', () => {
    const a = { identity: [], descriptors: [], noise: new Set<string>(), raw: new Set<string>() };
    const b = { identity: [], descriptors: [], noise: new Set<string>(), raw: new Set<string>() };
    expect(weightedSimilarity(a, b)).toBe(0);
  });
});
