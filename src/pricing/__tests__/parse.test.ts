...
describe("cleanTitle", () => {
  test('strips "Opens in a new window or tab"', () => {
    expect(
      cleanTitle("Xbox Series X Console Opens in a new window or tab"),
    ).toBe("Xbox Series X Console");
  });

  test('strips "New listing" prefix', () => {
    expect(cleanTitle("New listingApple iphone 16 Pro 128gb Unlocked")).toBe(
  ...
  test("strips multiple noise phrases", () => {
    const title =
      "New listingXbox One S 1Tb ConsoleOpens in a new window or tab";
    expect(cleanTitle(title)).toBe("Xbox One S 1Tb Console");
  });

  test("returns clean titles unchanged", () => {
    expect(cleanTitle("Nintendo Switch OLED 64GB")).toBe(
...
describe("parsePostageFromText", () => {
  test("Free delivery → 0, known", () => {
    expect(parsePostageFromText("Free delivery")).toEqual({
      postage: 0,
      postageKnown: true,
    });
  });
  ...
  test("Free delivery Click & Collect → 0, known", () => {
    expect(parsePostageFromText("Free delivery Click & Collect")).toEqual({
      postage: 0,
      postageKnown: true,
    });
  });
  ...
  test("Free delivery in 2 days → 0, known", () => {
    expect(parsePostageFromText("Free delivery in 2 days")).toEqual({
      postage: 0,
      postageKnown: true,
    });
  });
  ...
  test("Free delivery in 3 days Click & Collect → 0, known", () => {
    expect(
      parsePostageFromText("Free delivery in 3 days Click & Collect"),
    ).toEqual({ postage: 0, postageKnown: true });
  });

  test("+£2.94 delivery → 2.94, known", () => {
    expect(parsePostageFromText("+£2.94 delivery")).toEqual({
      postage: 2.94,
      postageKnown: true,
    });
  });
  ...
  test("+£5.88 delivery → 5.88, known", () => {
    expect(parsePostageFromText("+£5.88 delivery")).toEqual({
      postage: 5.88,
      postageKnown: true,
    });
  });
  ...
  test("Postage not specified → 0, unknown", () => {
    expect(parsePostageFromText("Postage not specified")).toEqual({
      postage: 0,
      postageKnown: false,
    });
  });
  ...
  test("Free collection in person → 0, known", () => {
    expect(parsePostageFromText("Free collection in person")).toEqual({
      postage: 0,
      postageKnown: true,
    });
  });
  ...
  test("empty string → 0, unknown", () => {
    expect(parsePostageFromText("")).toEqual({
      postage: 0,
      postageKnown: false,
    });
  });
  ...
  test("delivery in 2-3 days → 0, known (no cost, just time estimate)", () => {
    expect(parsePostageFromText("delivery in 2-3 days")).toEqual({
      postage: 0,
      postageKnown: true,
    });
  });
  ...
  test("delivery in 3 days Click & Collect → 0, known", () => {
    expect(parsePostageFromText("delivery in 3 days Click & Collect")).toEqual({
      postage: 0,
      postageKnown: true,
    });
  });
  ...
  test("long seller blurb ending in Free delivery → 0, known", () => {
    const text =
      "Official Dyson Outlet | Free Delivery | 30 Day Returns Free delivery";
    expect(parsePostageFromText(text)).toEqual({
      postage: 0,
      postageKnown: true,
    });
  });
  ...
  test("FAST & FREE DELIVERY!! Free delivery → 0, known", () => {
    expect(
      parsePostageFromText("FAST & FREE DELIVERY!! Free delivery"),
    ).toEqual({ postage: 0, postageKnown: true });
  });

  test("+£41.39 delivery → 41.39, known", () => {
    expect(parsePostageFromText("+£41.39 delivery")).toEqual({
      postage: 41.39,
      postageKnown: true,
    });
  });
...
describe("isExcluded", () => {
  ...
  test('excludes item with "for parts" in title', () => {
    const item: RawListing = {
      title: "iPhone 16 for parts",
      priceText: "£50.00",
      condition: "",
      link: "",
      deliveryText: "",
  ...
  test('excludes item with "cracked screen" in title', () => {
    const item: RawListing = {
      title: "Apple iPhone 16 cracked screen",
      priceText: "£100.00",
      condition: "Pre-owned",
      link: "",
      deliveryText: "",
...
describe("isMultiVariant", () => {
  ...
  test("flags listing with price range", () => {
    expect(
      isMultiVariant(makeItem("Apple iPhone 16", "£576.99 to £764.99")),
    ).toBe(true);
  });
  ...
  test("flags listing with multiple capacity tokens in title", () => {
    expect(
      isMultiVariant(
        makeItem("Apple iPhone 16 - 128GB 256GB 512GB All Colours"),
      ),
    ).toBe(true);
  });

  test('flags "all colours" in title', () => {
    expect(isMultiVariant(makeItem("Apple iPhone 16 128GB all colours"))).toBe(
      true,
    );
  });
  ...
  test('flags "all colors" in title', () => {
    expect(
      isMultiVariant(makeItem("Samsung Galaxy S24 128GB all colors")),
    ).toBe(true);
  });
  ...
  test("does not flag single-variant listing", () => {
    expect(
      isMultiVariant(makeItem("Apple iPhone 16 128GB Black Unlocked")),
    ).toBe(false);
  });
  ...
  for (const file of files) {
    test(`${file}: unknown postage rate ≤ 5%`, () => {
      const raw = JSON.parse(
        fs.readFileSync(path.join(testDataDir, file), "utf-8"),
      );
      const items: RawListing[] = (
        Array.isArray(raw) ? raw : (raw.items ?? [])
      ).filter((i: RawListing) => i && i.title && i.priceText);

      const parsed = parseRawListings(items);
      const withDeliveryText = parsed.filter(
        (_, idx) => items[idx].deliveryText.trim() !== "",
      );
      const unknownPostage = withDeliveryText.filter((p) => !p.postageKnown);

      const rate =
        withDeliveryText.length === 0
          ? 0
          : unknownPostage.length / withDeliveryText.length;
      const pct = (rate * 100).toFixed(1);

      console.log(
        `  ${file}: unknown postage ${unknownPostage.length}/${withDeliveryText.length} (${pct}%)`,
      );
