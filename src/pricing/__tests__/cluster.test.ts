...
describe('clusterListings — centroid (no chain drift)', () => {
  test('assigns items to the best-matching centroid, not the first member', () => {
    ...
    const xboxX = makeListing('Xbox Series X 1TB', ['xbox', 'series', '1tb'], ['console']);
    const dyson = makeListing('Dyson HP07 Air Purifier', ['dyson', 'hp07'], ['air', 'purifier']);
    const xboxS = makeListing('Xbox Series S 512GB', ['xbox', 'series', '512gb'], ['console']);
  ...
  test('identical listings end up in the same cluster', () => {
    const listings = Array.from({ length: 4 }, () =>
      makeListing('Xbox Series X 1TB Console', ['xbox', 'series', '1tb'], ['console']),
    );
...
describe('clusterListings — junk and excluded filtering', () => {
  test('junk items are excluded from clusters', () => {
    ...
    const junk = makeListing('Shop on eBay', ['shop'], [], 20, { isJunk: true });
  ...
  test('excluded items are not clustered', () => {
    const real = makeListing('Xbox Series X', ['xbox', 'series'], []);
    const excluded = makeListing('Xbox Series X For Parts', ['xbox', 'series'], ['parts'], 50, {
      isExcluded: true,
    });
