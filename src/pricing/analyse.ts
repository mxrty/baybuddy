...
// Minimum similarity for cross-corpus group matching
const CROSS_CORPUS_THRESHOLD = 0.15;
// Majority fraction for centroid token inclusion
const CENTROID_MAJORITY = 0.5;

function buildGroupCentroid(group: PricingGroup): WeightedTokens {
  ...
  return {
    identity: [...identityCounts.entries()]
      ...
      .map(([t]) => t),
    descriptors: [...descriptorCounts.entries()]
      .filter(([, c]) => c >= threshold)
      .map(([t]) => t),
...
/**
 * Rate an active listing against sold reference groups by token similarity.
 * Used when the listing was not part of the clustering corpus (cross-corpus rating).
 */
export function rateListingVsSold(
  listing: ParsedListing,
  groups: PricingGroup[],
): ListingAssessment {
  let matchedGroup: PricingGroup | null = null;
  ...
  function search(gs: PricingGroup[]): void {
    for (const g of gs) {
      ...
      // Search children even when parent matched — deeper group may score higher
      search(g.children);
    }
  }
  search(groups);
  ...
  const { totalPrice } = listing;
  const { p25, p75 } = (matchedGroup as PricingGroup).stats;
  ...
  const prices = (matchedGroup as PricingGroup).items
    .map((l) => l.totalPrice)
    .sort((a, b) => a - b);
  const below = prices.filter((p) => p < totalPrice).length;
  const percentile = prices.length > 0 ? below / prices.length : null;
